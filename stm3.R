...
# 8. REGIONAL EFFECTS
# ==============================================================================
# Set reference category: North America (canonical NCR origin)
...
extract_effects_df <- function(effects_obj, model, K, meta_df) {
  regions <- levels(meta_df$region)
  results <- list()
  for (k in 1:K) {
...
effects_df <- extract_effects_df(effects, model_final, K_CHOSEN, meta) 
%>%
  mutate(
...
n_matched <- sum(!is.na(meta$region))
