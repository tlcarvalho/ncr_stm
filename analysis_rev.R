...
author_patterns <- c(
  `TALIAFERRO JW`     = "TALIAFERRO",
  `RIPSMAN NM`        = "RIPSMAN",
  `SCHWELLER RL`      = "SCHWELLER",
  `ROSE G`            = "ROSE",
  `WALTZ KN`          = "WALTZ",
  `MEARSHEIMER JJ`    = "MEARSHEIMER",
  `LOBELL SE`         = "LOBELL",
  `ZAKARIA F`         = "ZAKARIA",
  `WALT S`            = "WALT",
  `WOHLFORTH WC`      = "WOHLFORTH",
  `STERLING-FOLKER J` = "STERLING",
  `WENDT A`           = "WENDT",
  `MORAVCSIK A`       = "MORAVCSIK",
  `SNYDER J`          = "SNYDER J",
  `BUZAN B`           = "BUZAN",
  `CHRISTENSEN TJ`    = "CHRISTENSEN",
  `GILPIN RG`         = "GILPIN",
  `KEOHANE R`         = "KEOHANE",
  `MORGENTHAU HJ`     = "MORGENTHAU",
  `IKENBERRY J`       = "IKENBERRY",
  `JOHNSTON A`        = "JOHNSTON",
  `KATZENSTEIN P`     = "KATZENSTEIN",
  `MEIBAUER G`        = "MEIBAUER",
  `RATHBUN B`         = "RATHBUN|RATHBURN",
  `FOULON M`          = "FOULON",
  `GOTZ E`            = "GOTZ|GÖTZ",
  `JUNEAU T`          = "JUNEAU",
  `KITCHEN N`         = "KITCHEN"
...
non_realist_authors <- c(
  "WENDT A", "MORAVCSIK A", "BUZAN B",
  "KEOHANE R", "IKENBERRY J", "JOHNSTON A", "KATZENSTEIN P"
)

ppc_raw <- df |>
  filter(TI %in% south_titles, !is.na(CR))

for (nm in names(author_patterns))
  ppc_raw[[nm]] <- as.integer(str_detect(ppc_raw$CR, author_patterns[[nm]]))

ppc <- ppc_raw |>
  summarise(across(all_of(names(author_patterns)), sum)) |>
  pivot_longer(everything(), names_to = "Author", values_to = "Citations") |>
  mutate(
    realist   = if_else(Author %in% non_realist_authors, "Non-Realist", "Realist"),
    prop_cite = Citations / n_south
  )

plot_citations <- function(data, title) {
  ggplot(data, aes(x = reorder(Author, prop_cite), y = prop_cite)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    theme_minimal(base_size = 10) +
    scale_y_continuous(limits = c(0, 1)) +
    labs(x = NULL, y = "Proportion of articles citing", title = title) +
    theme(plot.title = element_text(face = "bold"))
}

p_realist     <- plot_citations(filter(ppc, realist == "Realist"),     "Realists")
p_non_realist <- plot_citations(filter(ppc, realist == "Non-Realist"), "Non-Realists")

p_realist + p_non_realist +
  plot_annotation(
    caption = sprintf(
      "Source: Global South NCR articles (n = %d).", n_south
    )
  )

ggsave("figure_cited_authors.png", dpi = 300, height = 5, width = 10)


# A5. Publication outlets ─────────────────────────────────────────────
...
df |>
  ...
  labs(
    x       = NULL,
    y       = "Articles",
    caption = "Source: NCR bibliometric corpus (N = 349). Journals with > 5 articles shown."
  ) +
  theme(plot.title = element_text(face = "bold"))

ggsave("figure_journals.png", dpi = 300, height = 5, width = 8)
